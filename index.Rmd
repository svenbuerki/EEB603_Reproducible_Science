...
- This syllabus is available in two formats with **identical content**: the [web version](https://svenbuerki.github.io/EEB603_Reproducible_Science/index.html) and a [pdf version](index_syllabus_pdf.pdf) you can download and keep. **If you use assistive technology, the web version is the recommended one** -- it works better with screen readers and lets you adjust text size, spacing and contrast in your browser. Nothing is missing from either; use whichever suits you.
...
| **Report an error or ask a question** | <https://github.com/svenbuerki/EEB603_Reproducible_Science/issues/new/choose> |
...
- **Please book time with me while you work on both of your major assignments.** My [student hours](#Instructor) are by appointment precisely so that we can find a slot that suits you, in person or on Zoom. Whether you are designing your [bioinformatics tutorial](#BioTut) or building your [individual project](#arc2), coming to talk through your ideas, your data, or a piece of code that will not run is the single most useful thing you can do -- and the students who do it early consistently produce better work than those who arrive with a finished draft. You do not need a polished question; "I am stuck" is a perfectly good reason to email me.
...
# <a name="Description"></a>Course Description, Goals and Learning Outcomes {#Description}
...
> **The goal of this course is to equip students with the theoretical knowledge and bioinformatics tools necessary to enhance transparency, reproducibility, and efficiency in scientific research.**

Students will work throughout in open-source software: [R](https://cran.r-project.org), [RStudio](https://www.rstudio.com) and [R Markdown](https://rmarkdown.rstudio.com) (incl. [knitr](https://kbroman.org/knitr_knutshell/)). To develop these skills further, and to strengthen communication and teaching, students will do two things:

1. **Design bioinformatics tutorials and teach them to peers.**
2. **Design, implement, and present an individual project** aimed at developing a reproducible workflow tailored to their research interests.

Together, these give students key knowledge to **gather, store, share, prepare, and analyze data, and to communicate results** to the scientific community and various stakeholders (Figure \@ref(fig:part2)).

The rest of this section sets out what students will be able to do by the end ([Course Learning Outcomes](#Outcomes)), how the semester is organized ([Course Structure](#Structure)), and the topics covered ([Course Content](#courseCont)).
...
- **Uploading your assignments.** Everything lives in that one folder, organised into subfolders: put your [bioinformatics tutorial](#BioTut) in [`Bioinformatics_tutorials`](https://drive.google.com/drive/folders/1eZyjGQ478vaKiGD01hcryyNtdGp9t1J7?usp=drive_link), and your [individual project](#arc2) in [`Individual_reports`](https://drive.google.com/drive/folders/1eZyjGQ478vaKiGD01hcryyNtdGp9t1J7?usp=drive_link). Both links open the main folder, so you can see how it is laid out and find the subfolder you need.
...
## <a name="InstallSoftware"></a>Installing Software {#InstallSoftware}
...
## <a name="Assess"></a>Assessments {#Assess}
...
**[Arc 1 -- Bioinformatics for Reproducible Science](#arc1) (PARTS 1--2), 300 points.** You *learn* the theory and the tools, you *build* a tutorial that implements one stage of the scientific process, and you *teach* it to your peers. Each group takes a different stage, so by the time every tutorial has been taught, the class has worked through the whole process from importing data to communicating results -- and each of you will have taught one stage and been taught the rest.

1. [Participate and engage](#Participation) in class sessions across PARTS 1 and 2 (50 points).
2. Produce a [bioinformatics tutorial](#BioTut) based on a chapter from [Course Content - PART 2](#courseCont), normally as a group (150 points).
3. [Teach that tutorial](#TeachTut) to your peers over two sessions (100 points).

**[Arc 2 -- The Individual Project](#arc2) (PART 3), 300 points.** The same sequence applied to your own research: you *plan* a piece of reproducible work, you *build* it, and you *communicate* it.

4. A mandatory [project proposal](#Proposal), due about a third of the way through the semester (50 points).
5. The [final report](#FinalReport) (150 points).
6. The [oral presentation](#Oral) in the final week (100 points).

Together these total **600 points**, which are converted to a letter grade using the scale in the [Grading Policy](#Grading) below (Table \@ref(tab:mygrade)).

**There are no proctored exams or proctored assessments in this course.** Your grade is based entirely on the six assessments above, and every [Course Learning Outcome](#Outcomes) is assessed in at least one of them (Table \@ref(tab:loalign)). **Six of the seven outcomes are assessed in both arcs**, so that you meet them first on shared course material and then again on your own research; only outcome 6, designing and teaching a tutorial, belongs to the first arc alone. That double coverage matters most for **outcomes 1 and 2, on understanding the reproducibility crisis**: you first meet them in the PART 1 readings, but the [proposal](#Proposal) then asks which part of the reproducibility problem your project addresses, and the [report](#FinalReport) asks you to assess the reproducibility of existing work. Understanding why reproducibility fails is not background reading in this course -- it is what your individual project is built on.

```{r loalign, echo=FALSE}
loalign <- data.frame(
  Assessment = c(
...
knitr::kable(loalign,
  caption = "Alignment of each assessment with the Course Learning Outcomes, and the arc it belongs to.",
  format = "html", row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

The full rubrics for each assessment, together with a detailed outcome-by-outcome breakdown, are on the [Assignments](Assignments.html) page.

## <a name="Grading"></a>Grading Policy {#Grading}

The grading scale for this course is in Table \@ref(tab:mygrade). It applies to the **600 points** across the six [assessments](#Assess) described above.
...
## <a name="arc1"></a>Arc 1 --- Bioinformatics for Reproducible Science (300 points) {#arc1}

This arc runs across [PARTS 1 and 2](#Structure) and is assessed in three components: being present and engaged while the theory and tools are taught, building a bioinformatics tutorial, and teaching it to your peers.

The tutorials are not independent exercises. Each one implements a different stage of the scientific process --- importing and organizing data, tidying it, modelling it, presenting it in tables and figures, and sharing it through version control (see [Course Content -- PART 2](#courseCont) and Figure \@ref(fig:part2)). Taken together they cover the whole workflow, so **when the last tutorial has been taught, the class has built a complete, reproducible implementation of the scientific process** --- one that each of you has contributed a piece of and can now carry into your own research.
...
## <a name="Report"></a><a name="arc2"></a>Arc 2 --- The Individual Project (300 points, in three stages) {#arc2}
...
### <a name="FinalReport"></a>Final Report (150 points) {#FinalReport}
...
This is the **third and final stage of your [individual project](#arc2)**, following the proposal and the report. Its evaluation form records the combined total across all three stages (300 points).
...
# Points are deliberately omitted here -- they are in Table \@ref(tab:loalign)
# and on the Assignments page. Keeping this table narrow stops it overflowing
# the right margin of the pdf.
ai <- data.frame(
  "Assessment" = c("Participation and engagement",
                   "Bioinformatics tutorial",
                   "Teaching the tutorial",
                   "Project proposal",
                   "Final report",
                   "Oral presentation"),
  "AI use" = c("Not applicable",
               "Limited",
               "Not during teaching",
               "Permitted",
               "Permitted",
               "Preparation only"),
  "Core condition" = c("Contribute in your own words",
                       "You write and knit the code",
                       "You answer questions live",
                       "Disclose use; the plan is yours",
                       "Disclose use; research ethics",
...
knitr::kable(ai, caption = "Summary of generative AI expectations by assessment. The full conditions are given in the text below and in each assignment's instructions.", format = "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  pack_rows("ARC 1 -- Bioinformatics for Reproducible Science", 1, 3) %>%
  pack_rows("ARC 2 -- Individual Project", 4, 6)
```
...
Across all three stages of your individual project --- the [proposal](#Proposal), the [final report](#FinalReport) and the [oral presentation](#Oral) --- **you may use generative AI tools**, provided that your use is consistent with scientific integrity and research ethics. This is deliberate: AI tools are becoming part of the research workflow you will enter after this course, and learning to use them well -- critically, transparently, and reproducibly -- is itself a reproducible-science skill.
