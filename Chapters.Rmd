...
In Part A, students engage in self-reflection and group activities to explore the meaning of reproducible science and how it is implemented in research and scientific publications.

**Disclaimer:** The instructor has provided definitions and other supporting materials (in collapsible boxes) throughout this section. However, students are advised **not** to read these materials in advance in order to complete the exercises sincerely and gain the most from the experience.  

### Class Outline

This class taking place over 2-3 sessions is outlined as follows:
...
#### Individual self-reflection (5 minutes)

Students individually reflect on what "reproducible science" means to them and write down a definition. 

#### Group Sharing and Discussion (15 minutes)

- In small groups (3-4 students), students share their definitions and compare them

- Each group share their findings with the class to emulate a class discussion

#### Formal Definitions (10 minutes)

A set of standard definitions is provided to emphasize the use of same data/code for reproducibility versus new data/similar methods for replicability. Students are encouraged to engage and discuss these definitions in light of their research/knowledge/experience. 
    
<details>
    <summary><b>Reproducible Science</b></summary>
      <p>Reproducible science refers to the ability to obtain the same research findings and conclusions by using the same original data, computational steps, methods, and code used in a prior study.</p>
<p>Key elements of reproducible science:</p>
<ul>
  <li><strong>Transparency:</strong> All data, code, and methodological details are openly available and well-documented.</li>
  <li><strong>Documentation:</strong> Clear and detailed descriptions of the experimental design, procedures, analysis steps, and any decisions made during the research process.</li>
  <li><strong>Data Availability:</strong> Raw data used in the study is accessible and ideally deposited in publicly available repositories.</li>
  <li><strong>Code Availability:</strong> Software code and scripts used for analysis are shared and well-commented.</li>
  <li><strong>Use of Standardized Tools and Practices:</strong> Utilizing established tools and adhering to best practices within the research field to reduce variability and errors.</li>
</ul>
</details>

<details>
    <summary><b>Reproduction vs. Replication</b></summary>
      <p>**Reproduction** of a study means running the same computation (mostly referred as code in this class) on the same input data, and then checking if the results are the same. Reproduction can be considered as software testing at the level of a complete study.</p>
      <p>**Replication** of a scientific study (computational or other) means repeating a published protocol, respecting its spirit and intentions, but varying the technical details. For instance, it would mean using a protocol aiming at extracting genomic DNA developed on tomato and applying it on another species (e.g., sagebrush). For computational work, this would mean using different software, running a simulation from different initial conditions, etc. The idea is to change something that everyone believes should not matter (e.g. both tomato and sagebrush are plants and have DNA), and see if the scientific conclusions are affected or not.</p>
      <p>Overall, **reproduction verifies that a computation was recorded with enough detail that it can be analyzed later or by someone else**. On the other hand, **replication explores which details matter for reaching a specific scientific conclusion**. A replication attempt is most useful if reproducibility has already been verified. Otherwise, if replication fails, leads to different conclusions, you cannot trace back the differences in the results to the underlying code and data.</p>
</details>

### Case Studies and Challenges {#CaseStudies}

#### Group Work Activity (50 minutes)

- Each group receives a publication illustrating a challenge or challenges to reproducibility (e.g., lack of data sharing, insufficient methodological details, issues with statistical analysis, publication bias)
- Each group is tasked to analyzes their assigned case study by:
    - Reading the publication
    - Identifying the reproducibility challenge(s)
    - Discussing the potential impact on the research community
    - Brainstorming possible solutions or ways the situation could be mitigated to improve reproducibility
- Students are encouraged to research their publication online and use the [resources](#OnlineResChap1) presented below to support their findings and group activity. **Please don't research our publication prior to making your own conclusions.** 

#### Online Resources {#OnlineResChap1}

This list of online resources are provided to support your group activity:
...
#### Group Presentations and Class Discussion (20 minutes)

- Each group presents their findings on their assigned case study, and proposed solutions
- **Tip for Discussion:** We facilitate a class discussion by drawing connections between the different case studies and exploring the systemic challenges to reproducibility and the various factors at play.

### Best Practices and Open Science Tools {#BestPract}

#### Group Activity (20 minutes)

- Groups brainstorm best practices and tools to improve research reproducibility, based on their understanding and previous discussions
- Class discussion to present our recommendations to implement reproducible science

#### Key Strategies and Tools (10 minutes)
...
- Students reflect individually on:
  - One key takeaway from this exercise
  - One action that they can implement in their research to improve reproducibility

#### Class Discussion & QA (10 minutes)

- Students share their reflections and ask any pending questions
...
The `pdf` of the presentation can be downloaded [here](Chapters_files/Chapter_1_Part_B_Reproducibility_crisis.pdf). The `pdf` of @Baker_2016 is available in our shared Google Drive at this path:
...
- https://github.com/svenbuerki/EEB603_Reproducible_Science/tree/master/Exercises/Chapter_1
...
If you have difficulties accessing the Google Drive, click [here](https://drive.google.com/drive/folders/1JnGop4WBuToNDNFf7fgLthLDJ-re36o3?usp=drive_link).
...
**NOTE:** The instructor is using the following version of `RStudio`: Version 2022.12.0+353 (2022.12.0+353). If your computer is experiencing issues running the latest version of the software, you can install previous versions [here](https://docs.posit.co/previous-versions/rstudio/).
...
1. Create an `*.Rmd` file entitled `Exercises chapter 2: part A` and select `HTML` as output format.
2. Save this document as `Exe_chap2_partA.Rmd` in a folder called `Exercises` located in:
