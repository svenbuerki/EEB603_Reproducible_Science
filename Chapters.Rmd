...
2. Add a first-level header titled `Tables`.  
...
2. Add a first-level header titled `Figures`.  
3. Insert an R code chunk under your header by clicking the `Insert` button ![](Figures/insert.png) in the editor toolbar.
4. Copy and paste the following R code into your code chunk:
```{r eval=FALSE}
...
This tutorial is devoted to part C of chapter 1 and provides students with opportunities to learn procedures to (see Figure \@ref(fig:RMarkdwnstrct)):
...
The slides presented in class can be downloaded [here](Chapters_files/Chapter_01_PartC.pdf). All the information presented in these slides are found in the text below.
...
To improve code reproducibility and efficiency and to follow publication requirements, it is customed to include a "code chunk" at the beginning of your `.Rmd` file to set global options applying to the whole document (see Figure \@ref(fig:RMarkdwnstrct)). Those settings are related to the following elements of your code:
...
### Apply global changes to all code chunks {#setupcode}

In this section, we will collate all global settings discussed above into a code chunk entitled `setup`, which will be placed under the YAML metadata section (see Figure \@ref(fig:RMarkdwnstrct) for more details on location). In addition to containing the global settings, it is advisable to also include a code section devoted to loading required R packages (see Chapter 1 - part B and Figure \@ref(fig:RMarkdwnstrct)). 
