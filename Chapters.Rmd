...
1. Apply the function to a single value:
...
2. Apply the function to a vector containing multiple values:
...
<div class="info">
  ...
  <ul>
    <li><b>R Script:</b> Create an <code>R</code> script titled <code>R_lists.R</code> and save it in your project folder.</li>
...
Lists are **R objects that can contain elements of different types**, such as numbers, strings, vectors, or even another list. A list can also include a matrix or a function as an element. Lists are created using the `list()` function.

#### How to Create a List {#HowCreateList}

Below is an example of how to create a list containing strings, numbers, vectors, and logical values:
...
List elements can be assigned names, which allows them to be accessed using those names, as shown below:
...
Elements of a list can be accessed using their **index** or, in the case of named lists, using their **names**. 
...
We will now apply what you have learned to return multiple values in a UDF.
...
We are tasked with **developing, implementing, and applying a UDF to calculate both the logarithm and the square of a number.**
...
#### Student Activity and Solution <span style="color:gray;">(20 minutes)</span> {.tabset}
...
To demonstrate how to apply defensive programming in your code, a power function `y = x^n` is implemented as a user-defined function (UDF) and used as an example.
...
<div style="text-align: center;">
  <button onclick="toggleFullScreen()">Toggle Full Screen</button>
  <iframe id="pdfFrame" src="Chapters_files/Chapter_3_Scientific_process.pdf" width="100%" height="600px" style="border:none;">
