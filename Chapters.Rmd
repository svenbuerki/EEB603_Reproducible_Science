...
<div style="text-align: center;">
  <button id="toggleButton" onclick="toggleFullScreen()">Enter Full Screen</button>
  <iframe id="pdfFrameUnique" allowfullscreen src="Chapters_files/Chapter_3_Scientific_process.pdf" width="100%" height="600px" style="border:none;">
    <p>Your browser does not support PDFs. <a href="Chapters_files/Chapter_3_Scientific_process.pdf">Download the PDF</a>.</p>
  </iframe>
</div>

<script>
function toggleFullScreen() {
  var elem = document.getElementById("pdfFrameUnique");
  var button = document.getElementById("toggleButton");
  
  // Refresh iframe source with timestamp to avoid caching issues
  var pdfUrl = "Chapters_files/Chapter_3_Scientific_process.pdf?" + new Date().getTime();
  elem.src = pdfUrl;
...
// Update button text and refresh on fullscreen state change (handles Esc key exits)
document.addEventListener("fullscreenchange", function() {
  ...
  if (document.fullscreenElement) {
    ...
    // Refresh iframe on exit to ensure clean fallback
    elem.src = "Chapters_files/Chapter_3_Scientific_process.pdf?" + new Date().getTime();
  }
});

// For older browser compatibility (optional, but harmless)
...
function updateButton() {
  var button = document.getElementById("toggleButton");
  button.textContent = document.fullscreenElement ? "Exit Full Screen" : "Enter Full Screen";
}
</script>
...
The Transparency and Openness Promotion (TOP) guidelines [published in @Nosek1422] provide a framework for journals and funders to incentivize or require greater transparency in the planning and reporting of research. The guidelines consist of eight modular standards, each with three levels of increasing stringency (see Figure \@ref(fig:OSF)). Journals choose which of the eight standards to adopt and at what level, allowing flexibility that accommodates disciplinary differences while establishing community-wide transparency standards.
...
In this chapter, we will investigate both the [CARE Principles](#CAREPrinc) and [Data Sovereignty](#datasov), emphasizing how these concepts are closely connected.
...
<div style="text-align: center;">
  <button id="toggleButton" onclick="toggleFullScreen()">Enter Full Screen</button>
  <iframe id="pdfFrameDM" allowfullscreen src="Chapters_files/Chapter_5_PartA.pdf" width="100%" height="600px" style="border:none;">
    <p>Your browser does not support PDFs. <a href="Chapters_files/Chapter_5_PartA.pdf">Download the PDF</a>.</p>
  </iframe>
</div>

<script>
function toggleFullScreen() {
  var elem = document.getElementById("pdfFrameDM");
  var button = document.getElementById("toggleButton");
  
  // Refresh iframe source with timestamp to avoid caching issues
  var pdfUrl = "Chapters_files/Chapter_5_PartA.pdf?" + new Date().getTime();
  elem.src = pdfUrl;
...
document.addEventListener("fullscreenchange", function() {
  ...
  if (document.fullscreenElement) {
    ...
    button.textContent = "Enter Full Screen";
    // Refresh iframe on exit to ensure clean fallback
    elem.src = "Chapters_files/Chapter_5_PartA.pdf?" + new Date().getTime();
  }
});
...
The following steps must be fully integrated to produce reproducible code:
...
In this chapter, we will cover Steps 1 to 4. Step 5 was introduced in Chapter [2](#chap2), and Step 7 is covered in Chapter [5](#partDM) under Data Management. Step 6 will be addressed in the bioinformatics tutorial associated with Chapter 12.
...
<div style="text-align: center;">
  <button id="toggleButton" onclick="toggleFullScreen()">Enter Full Screen</button>
  <iframe id="pdfFrameRC" allowfullscreen src="Chapters_files/Chapter_5_Reproducible_code.pdf" width="100%" height="600px" style="border:none;">
    <p>Your browser does not support PDFs. <a href="Chapters_files/Chapter_5_Reproducible_code.pdf">Download the PDF</a>.</p>
  </iframe>
</div>

<script>
function toggleFullScreen() {
  var elem = document.getElementById("pdfFrameRC");
  var button = document.getElementById("toggleButton");
  
  // Refresh iframe source with timestamp to avoid caching issues
  var pdfUrl = "Chapters_files/Chapter_5_Reproducible_code.pdf?" + new Date().getTime();
  elem.src = pdfUrl;
...
document.addEventListener("fullscreenchange", function() {
  ...
  if (document.fullscreenElement) {
    ...
    button.textContent = "Enter Full Screen";
    // Refresh iframe on exit to ensure clean fallback
    elem.src = "Chapters_files/Chapter_5_Reproducible_code.pdf?" + new Date().getTime();
  }
});
...
The dataset for this chapter is contained in the `Project_ID` folder, available via our shared [Google Drive](https://drive.google.com/drive/folders/1ZNNfDcn6b1vLscjbg66aU5cWtx773lnw?usp=drive_link):

**Path:**  
`Reproducible_Science > Exercises > Chapter_5 > Project_ID`

Please download the entire `Project_ID` folder to your local machine before starting the exercises.

#### R Packages {#RpkgRepCode}

To ensure all required R packages are available, we will reuse code introduced in Chapter 1. The exercises rely on the following packages:

- `data.tree`
- `DiagrammeR`

Please make sure these packages are installed and loaded in your R environment before proceeding. This cane be done by using the R code provided below. Before proceeding read the disclaimer.
...
<div class="disclaimer">
  <p><strong>💡 Disclaimer</strong></p>
  <ul>
    <li><strong>Warning:</strong> The code in this section will install the following <code>R</code> packages (including their dependencies) on your computer: <code>data.tree</code> and <code>DiagrammeR</code>.</li>
    <li><strong>About the <code>R</code> packages:</strong> These packages are required to help you organize your project for reproducibility. Both are available on <a href="https://cran.r-project.org/web/packages/" target="_blank" rel="noopener noreferrer">CRAN</a>, the official repository for <code>R</code> packages.</li>
    <li><strong><code>R</code> package repositories:</strong> Before running the installation code, ensure that your <code>R</code> package repositories are properly set in RStudio. You can follow <a href="https://docs.posit.co/ide/user/ide/guide/environments/r/packages.html" target="_blank" rel="noopener noreferrer">this guide</a> for detailed instructions.</li>
...
Code to install R packages for this section of Chapter 5: 
...
Organizing your project for reproducibility begins with drawing a workflow that will guide your project implementation. An example of a reproducible project workflow is displayed in Figure \@ref(fig:ReproWorkflow). This workflow will be used as a template for the material presented in this chapter.
