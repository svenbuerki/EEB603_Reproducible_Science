...
- @Kerr1998 - On HARKing (see Glossary section).
- @Nosek2012 - On incentives.
- @Nosek1422 - On promoting an open research culture.
- @Baker_2016 - Results of reproducible survey.
- @Munafo - Provides framework for measures proposed in this chapter.
- @Nosek2600 - On the pre-registration revolution.
- @Troudet2018 - On accuracy of biodiversity occurrences available in GBIF.
...
The Center for Open Science also proposes a platform called "Open Science Framework" or OSF to achieve the same goal (\autoref{fig:OSF}). [OSF](https://cos.io/our-products/osf/) is a free and open source project management repository that supports researchers across their entire project life-cycle. As a collaboration tool, OSF helps researchers work on projects privately with a limited number of collaborators and make parts of their projects public, or make all the project publicly accessible for broader dissemination with citable, discoverable DOIs. As a workflow system, OSF enables connections to the many products researchers already use to streamline their process and increase efficiency.

With OSF's workflow and storage integrations, researchers can really manage their entire projects from one place. The OSF workflow connects the valuable research tools researchers are already using, so that they can effectively share the story of their research projects and eliminate data silos and information gaps (\autoref{fig:OSF}). OSF ecosystem is designed to allow all those tools to work together the way researchers' do, removing barriers to collaboration and knowledge (\autoref{fig:OSF}).
...
The presentation slides for Chapter 4 - part A can be downloaded [here](Chapters_files/Chapter_4_Data_management.pdf).
...
The presentation slides for Chapter 4 - part B can be downloaded [here](Chapters_files/Chapter_4_Reproducible_code.pdf).
...
Gathering data can be done by either importing locally stored data sets (= files stored on your computer) or by importing data sets from the Internet. Usually, these data sets are saved in plain-text format (usually in comma-separated values format or `csv`) making importing them into R a fairly straightforward task. However, if data sets are not saved in plain-text format, the users will have to start by converting them. In most cases, data sets will be saved in `xls` or `xlsx` formats and functions implemented in the [*readxl*](https://readxl.tidyverse.org/) package [@readxl] will be used. Although not covered in this chapter, if your data sets were created by other statistical programs such as SPSS, SAS or Stata, these can be imported into R using functions from the *foreign* package [@foreign]. Finally, data sets could be saved in compressed documents, which will have to be processed prior to importing the data into R.
...
Here, we are providing an overview of the methods and functions that will be used in this chapter to manage your projects and gather, convert and clean your data. Ultimately these tools will be applied to document and produce the raw data at the basis of your research.

## Learning outcomes

- Creating RStudio projects to manage your reproducible project.
- Importing locally stored data sets.
- Importing data from the Internet focusing on GitHub.
- Converting non plain-text to plain text formats.
- Un-compressing content of whole document and listing and then un-compressing specific files in compressed document. 
...
In this section, we will be focusing on learning procedures to import data from the internet focusing on GitHub. More precisely, we will be learning procedure to:

1. Import comma-separated values format (`csv`) stored in GitHub repositories.
2. Download whole GitHub repositories.

### GitHub repositories used in this chapter

#### EEB603_Reproducible_Science repository

This repository is dedicated to this course and is used to demonstrate the procedure to import `csv` files from GitHub repositories. More specifically, we will be importing different versions of the file (`Timetable_EEB603_topic_tasks.csv`) at the origin of the [Timetable](Timetable.html). It is located at this URL (Uniform Resource Locator): 

- https://github.com/svenbuerki/EEB603_Reproducible_Science 

#### Sagebrush_rooting_in_vitro_prop

This repository is associated to @Barron2020 and is used to demonstrate procedure to download whole GitHub repositories. It is located at this URL: 

- https://github.com/svenbuerki/Sagebrush_rooting_in_vitro_prop

### Importing comma-separated values format from GitHub into R

To achieve this task and import `csv` files deposited on GitHub into R the following tasks have to be done:

1. Retrieve the *raw

#### The SHA-1 hash accession number

With the growing popularity of GitHub, several authors are depositing their data sets on GitHub and you might would like to access those for your research. Since Git and GitHub are supporting version control, it is important to report the exact version of the file or data set that you have downloaded. To support such feature, each version of a file/data set is associated to a unique encrypted [SHA-1](https://en.wikipedia.org/wiki/SHA-1) (Secure Hash Algorithm 1) hash accession number. This means that if the file changes (because the owner of the repository updated it), its SHA-1 hash accession number will change. Such feature allows users to referring to the exact file/data set, which was used in their analyses therefore supporting reproducibility.  
...
Now that we have retrieved and shortened the *raw* GitHub URL pointing to our target `csv` file, we can use the *source_data()* function implemented in the R package *repmis* [@repmis] to download the file. This can be done as follows:
...
#### Retrieving SHA-1 hash from previous commit
...
1. Import locally stored csv data set.
2. Import locally stored Excel (xls) data set.
3. Convert excel into csv formats.
4. 


```{r}
url <- "https://raw.githubusercontent.com/svenbuerki/Sagebrush_rooting_in_vitro_prop/master/01_Raw_Data/1_block_8_12_2020%20-%201_block.csv"
```
...
Gather, convert, clean and merge data
...
2. Importing locally stored data sets
  - Plain-text format:
    - csv
  - Non-plain text formats:
    - xls/xlsx, S, SPSS, etc.
  - Convert non-plain-text to plain text formats
    - Example with xls to csv
  - Open csv from zip files (witout uncompressing): https://stackoverflow.com/questions/12460938/r-reading-in-a-zip-data-file-without-unzipping-it
  
```{r echo=F, eval=F}
filesZip <- as.character(unzip("Chapters_files/drive-download-20211009T232918Z-001.zip", list = TRUE)$Name)

#only unzip 1 specific file
unzip("Chapters_files/drive-download-20211009T232918Z-001.zip", files = filesZip[1])


read_xlsx(unz("Chapters_files/drive-download-20211009T232918Z-001.zip", filesZip[1]), sheet=1, skip = 2)

```
3. Importing data sets form the internet

- Non-secure URLs (maybe not or just mention that can be open with read.table())
- Secure URLs
