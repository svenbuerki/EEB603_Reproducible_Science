...
- [Study and master the structure of an R Markdown document](#structRMD)
- [Reinforce your expertise in creating R Markdown documents in dedicated project folders](#creatRMDfile)
- [Install R dependencies to produce the document](#instDep)  
- [Define parameters in the YAML metadata section to control the formatting of the knitted document](#YAMLset)  
- [Load the required R packages for your analysis](#loadRreq)  
- [Generate a citation file for all R packages used to produce the report](#citfileRpkg)  
- [Generate an appendix with citations for all R packages](#appendixRpkg) 
- [Generate an appendix with R package versions used to produce the report](#appendixRverOS)
...
### Install R Dependencies {#instDep}
...
The best procedure to ensure reproducibility would be to copy the code below in an R script file (entitled `01_R_dependencies.R`) saved at the root of your working directory (if this directory does not exist yet, please start by creating it and naming it `Chapter_2_PartB`).

```{r pck, eval=TRUE, tidy=FALSE}
##~~~
# Check/Install R dependencies
##~~~
# This code is dedicated to packages for Chapter 2
...
The YAML metadata section (Figure \@ref(fig:RMarkdwnstrct)) allows users to provide arguments (referred to as fields) to convert their R Markdown document into its final form. In this class, we will be using functions implemented in the *knitr* [@R-knitr; @knitr2015] and *bookdown* packages [@R-bookdown; @bookdown2016] to populate this section (field names as declared in the YAML metadata section are provided between parenthesis):
...
Do the following steps to set your YAML metadata section (also see Figure \@ref(fig:RMarkdwnstrct)):
...
4. Edit the top YAML metadata section as shown in the [section](#YAMLset) above.  
...
- To learn more about YAML and its application to R Markdown, please visit this [website](https://zsmith27.github.io/rmarkdown_crash-course/lesson-4-yaml-headers.html).
...
### Load Required R Packages {#loadRreq}

It is best practice to add an R code chunk directly under the YAML metadata section to load all the required R packages used to produce your report (same code than presented [here](#instDep)) (see Figure \@ref(fig:RMarkdwnstrct)). This feature will also allow to automatically generate a citation file with all R packages used to generate your report (see below). Applying this approach will contribute to improving the reproducibility of your research!   

##### Step-by-step procedure

1. Include an R code chunk directly under your YAML metadata section (Figure \@ref(fig:RMarkdwnstrct)).
2. Name code chunk `packages` and set options line as follows:
  + `echo = FALSE`, 
  + `warning = FALSE`, 
  + `include = FALSE`.
3. Add the following code to load required packages to produce your report (or copy content of `01_R_dependencies.R`):
...
### Generate a Citation File of R Used Packages {#citfileRpkg}
...
Although a References section will be provided at the end of your document to cite in text references (see [References] and Figure \@ref(fig:RMarkdwnstrct)), it is customized to add citations for all R packages used to generate the research in [Appendix 1]. We will learn here the procedure to assemble such Appendix.

##### Step-by-step procedure

1. Include Appendices after the References section (see Figure \@ref(fig:RMarkdwnstrct)).
  + This is done by using `<div id="refs"></div>` as shown below, which allows printing Appendices (or any other material) after the References section (see [here](https://bookdown.org/yihui/rmarkdown-cookbook/bibliography.html#include-appendix-after-bibliography) for more details):
```{r eval=FALSE}
# References
...
```
2. Insert an R code chunk directly under `# Appendix 1` to read in and print citations saved in `packages.bib`. This is done as follows:
...
In addition to providing citations to R packages, you might also want to provide full information on R package versions and your operating systems (see Figure \@ref(fig:RMarkdwnstrct)). With R, the simplest (but a useful and important) approach to document your R environment is to report the output of `sessionInfo()` (or `devtools::session_info()`). Among other information, this will show all the packages and their versions that are loaded in the session you used to run your analysis. If someone
