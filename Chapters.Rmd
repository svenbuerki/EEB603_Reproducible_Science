...
I assign one of the retracted articles below to each group at the start of the session, and hand out copies in class. The links are here so that you can find your article again afterwards. The reasons behind each retraction are given at the end of this section --- **work through your article and reach your own conclusions before opening that table.**

```{r caseassign, echo=FALSE, results='asis', warning=FALSE, message=FALSE}
###~~~
# Case-study assignments.
#
# Groups 1, 2 and 4 are derived from the Retraction Watch csv used for Table
# \@ref(tab:retractionwatch), so the titles and DOIs here can never drift away
# from that table.
#
# Group 3's article has NOT been formally retracted, so it has no row in that
# csv and is defined here instead. If its DOI is left empty, the entry falls
# back to the PubPeer thread rather than emitting a broken doi.org link.
###~~~
PUBPEER <- "https://pubpeer.com/publications/0905A70AF6A77C4309B743FFCD21F8"

group3 <- data.frame(
  GroupID          = 3,
  Title            = "PubPeer discussion thread (article not formally retracted)",
  Journal          = "",
  OriginalPaperDOI = "")          # <- add the DOI here if you want to link the article itself

cols  <- c("GroupID", "Title", "Journal", "OriginalPaperDOI")
cases <- read.csv("Chapters_files/Chapter_1_PartA_Case_Studies.csv")[, cols]
cases <- rbind(cases, group3)
cases <- cases[order(cases$GroupID), ]

for (i in seq_len(nrow(cases))) {
  url <- if (nzchar(cases$OriginalPaperDOI[i])) {
    paste0("https://doi.org/", cases$OriginalPaperDOI[i])
  } else {
    PUBPEER
  }
  journal <- if (nzchar(cases$Journal[i])) sprintf(", *%s*", cases$Journal[i]) else ""
  cat(sprintf("- **Group %s** --- [%s](%s)%s\n",
              cases$GroupID[i], cases$Title[i], url, journal))
}
```
