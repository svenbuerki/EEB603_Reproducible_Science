...
This chapter focuses on mastering R Markdown syntax and protocols that allow the inclusion of text, code, figures, tables, and references within a single document. The resulting document can then be compiled into an output file—such as `PDF`, `HTML`, or `Word` to facilitate the sharing of your research.

This tutorial provides students with the foundational knowledge necessary to complete their bioinformatics tutorials (**PART 2**) and individual projects (**PART 3**). The chapter is divided into five parts:
...
### Our First R Markdown Document {#firstMDhow}
...
2. Copy the following files, available on [Google Drive](https://drive.google.com/drive/folders/1JFjKmb0JHm96UfimO8SJLq33hE1JRu5v?usp=drive_link), into your project folder:  
    ...
    + `AmJBot.csl`  
3. Edit the YAML metadata section (at the top of your R Markdown document) as shown in the [section above](#YAMLset)
...
2. Name the code chunk `packages` and set the chunk options as follows:
   + `echo = FALSE`  
   + `warning = FALSE`  
   + `include = FALSE`
3. Copy the following code into your R code chunk to install and load the packages required to produce your report:
...
I don’t know about you, but I often struggle to properly cite R packages in my publications. Fortunately, R provides a built-in function to help with this. If you want to retrieve the citation for an R package, you can use the base R function `citation()`.
...
To use citation entries generated by `toBibtex()`, copy the output to a `.bib` file and save it in your working directory. You will then be able to cite references from this file directly in your R Markdown document. 
...
This tutorial is devoted to Part B of Chapter 2 and provides students with opportunities to learn procedures to:

- [Insert tables in an R Markdown document](#inserttable)
- [Insert and figures in an R Markdown document](#insertfigure)
...
knitr::kable(grades, caption = "Grading scale applied in this class.") %>% kable_styling(c("striped","scale_down"))
```
5. Edit the code chunk options line by adding the following argument (**Note:** each argument should be separated by a comma):
  - `echo = FALSE`
6. Add the unique label `tabgrades` in the chunk options line (just after `{r`) to enable further cross-referencing.
7. Test your code to check that it produces the expected table by using the `Run` button. 
8. Knit your document using the `Knit` button ![](Figures/knit_2.png) on the editor toolbar (Figure \@ref(fig:knitout)). 
...
7. Test your code to check that it produces the expected plot by using the `Run` button. 
8. Knit your document using the `Knit` button ![](Figures/knit_2.png) on the editor toolbar (Figure \@ref(fig:knitout)). 
