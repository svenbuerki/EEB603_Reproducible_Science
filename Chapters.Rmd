...
The **aim** of this tutorial is to provide an overview of preocedures that can be applied to streamline your R Markdown document to support your computing needs and formatting style.
...
<!-- Carry on revising text from here -->
...
When inserting the above code into an R code chunk (see Figure \@ref(fig:RMarkdwnstrct)), please set the options of the chunk as follows:
...
### Exercise to work on learning objectives 
...
3. Insert an R code chunk (using the `Insert` ![](Figures/insert.png) button) directly under the YAML metadata section and entitle it `setup`. This code chunk will be used to define the global settings for the following options as implemented in the `opts_chunk$set()` function:
    - Text results.
    - Code decoration.
    - Caching code.
    - Plots.
    - Positioning of figures.
4. Copy the content of the code presented in this [section](#setupcode) inside of your `setup` code chunk. 
5. Use the R code provided [below](#step5) (which generates a plot) to learn more about the effect of global settings on code outputs.
6. Once the file is completed, compile your document using the `Knit` ![](Figures/knit_2.png) button. Please pay attention to the outputs in your folder (= working directory).
7. Use the `bookdown` `\@ref()` function to cite your figure/plot in the text.

#### R code for step 5 {#step5}

The R code provided below is associated with step 5 of the exercise and it produces the plot displayed in Figure \@ref(fig:PlotFig).
...
4. Go to step 6 in the above section and complete the exercise.
