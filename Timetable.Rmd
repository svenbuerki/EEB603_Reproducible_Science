...
The tentative timetable for this course is provided in Table \@ref(tab:timetab). Please note that the instructor may adjust our schedule to accommodate the course’s progress and students’ needs. Any changes will be communicated to enrolled students in a timely manner.

```{r timetab, echo=FALSE}
###~~~
# Build a data frame with all dates from beginning to end of Fall semester
###~~~
dates <- data.frame(seq.Date(from=as.Date("2025/08/25"), to=as.Date("2025/12/19"), by="day"))
...
datesclass <- data.frame(WeekID = sort(rep(seq(from=1, to=17, by=1), 2)), datesclass)

###~~~
# Exclude dates falling during holiday break
###~~~
#Establish vector of dates related to holidays
holiday <- seq.Date(from=as.Date("2025/11/24"), to=as.Date("2025/11/30"), by="day")
...
colnames(timetable)[5] <- "Homework"
# Discrad larst column with URLs
timetable <- timetable[,-ncol(timetable)]
###~~~
#Plot timetable in doc
###~~~
knitr::kable(timetable,  caption = "Tentative timetable for the EEB603 Reproducible Science course, Fall 2025 semester.") %>%
